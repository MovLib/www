...
class Index extends \MovLib\Presentation\Award\AbstractBase {
  ...
  protected function getPageContent() {
    global $i18n;
    $this->headingBefore =
      "<a class='btn btn-large btn-success fr' href='{$i18n->r("/award/{0}/event/create", [ $this->award->id ])}'>" .
    ...
    $noItemText .=
      $i18n->t("<p>Would you like to {0}create a new entry{1}?</p>", [
        "<a href='{$i18n->r("/award/{0}/category/creat", [ $this->award->id ])}'>",
        "</a>"
      ]);

    $moviesRoute = $i18n->rp("/award/{0}/event/{1}/movies", [ $this->award->id, "{{ id }}" ]);
    $seriesRoute = $i18n->rp("/award/{0}/event/{1}/series", [ $this->award->id, "{{ id }}" ]);

    return new AwardEventListing($result, $noItemText, "AwardEvent", $moviesRoute, $seriesRoute);
  }
