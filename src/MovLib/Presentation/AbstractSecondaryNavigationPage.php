...
abstract class AbstractSecondaryNavigationPage extends \MovLib\Presentation\Page {
  ...
  public function getContent() {
    ...
    // Instantiate the secondary navigation.
    $this->secondaryNavigation = new Navigation($this->id, $i18n->t("Secondary Navigation"), $this->getSecondaryNavigationMenuitems());

    // We need the unordered list for styling, check the CSS.
    $this->secondaryNavigation->unorderedList = true;
    ...
    return
      "<div class='container secondary-container'><div class='row'>" .
        "<aside class='secondary-navigation span span--2' role='complementary'>{$this->secondaryNavigation}</aside>" .
        "<div class='secondary-content span span--10'>{$pageContent}</div>" .
      "</div></div>"
    ;
  }
