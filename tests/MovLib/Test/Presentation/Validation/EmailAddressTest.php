...
class EmailAddressTest extends \PHPUnit_Framework_TestCase {
  ...
  public static function dataProviderValidPHP() {
    // IMPORTANT!
    //
    // The regular expression has to recognize any valid email address, it's no problem if it doesn't invalidate some of
    // the fancy invalid ones, that's something the server can take care of. But it has to let any and every valid
    // email address through to the server! ALWAYS!
    //
    // IMPORTANT!
  }

  /**
   * @covers \MovLib\Presentation\Validation\EmailAddress::validate
   * @dataProvider dataProviderInvalid
   */
  public function testPatternInvalid($email) {
    $pattern = str_replace("/", "\/", EmailAddress::PATTERN);
    $this->assertEquals(0, preg_match("/{$pattern}/", $email));
  }
  ...
  public function testPatternValid($email) {
    $pattern = str_replace("/", "\/", EmailAddress::PATTERN);
    $this->assertEquals(1, preg_match("/{$pattern}/", $email));
  }

  /**
   * @covers \MovLib\Presentation\Validation\EmailAddress::validate
   * @dataProvider dataProviderInvalidPHP
   * @expectedException \MovLib\Exception\ValidationException
   */
  public function testInvalid($email) {
    (new EmailAddress($email))->validate();
  }
  ...
  public function testValid($email) {
    $this->assertEquals($email, (new EmailAddress($email))->validate());
  }
