...
The following is the output of our current MariaDB installation. This gives you a full overview of all available
configuration options. Check out the `my.conf` in this folder to see our current configuration. Please note that the
...
```
mysqld  Ver 10.0.2-MariaDB-1~wheezy-log for debian-linux-gnu on x86_64 (mariadb.org binary distribution)
...
--defaults-extra-file=# Read this file after the global files are read.
  ...
  --aria[=name]       Enable or disable Aria plugin. Possible values are ON,
                      OFF, FORCE (don't start if the plugin fails to load).
  ...
  --aria-log-purge-type=name
                      ...
                      Possible values of name are "immediate", "external" and
                      "at_flush"
  ...
  --aria-recover[=name]
                      ...
                      repaired. Possible values are one or more of "NORMAL"
                      (the default), "BACKUP", "FORCE", or "QUICK".
  --aria-repair-threads=#
  ...
  --aria-stats-method=name
                      ...
                      should treat NULLs. Possible values are "nulls_unequal",
                      "nulls_equal", and "nulls_ignored".
  --aria-sync-log-dir=name
                      Controls syncing directory after log file growth and new
                      file creation. Possible values are "never", "newfile" and
                      "always").
  ...
  --federated[=name]  Enable or disable FEDERATED plugin. Possible values are
                      ON, OFF, FORCE (don't start if the plugin fails to load).
  --feedback[=name]   Enable or disable FEEDBACK plugin. Possible values are
                      ON, OFF, FORCE (don't start if the plugin fails to load).
  ...
  --innodb-analyze-is-persistent
                      ANALYZE TABLE in InnoDB uses a more precise (and slow)
                      sampling algorithm and saves the results persistently.
  ...
  --innodb-flush-neighbors
                      ...
                      (Defaults to on; use --skip-innodb-flush-neighbors to disable.)
  ...
  --innodb-ft-enable-diag-print
                      ...
                      (Defaults to on; use --skip-innodb-ft-enable-diag-print to disable.)
  ...
  --innodb-ft-index-table[=name]
                      ...
                      values are ON, OFF, FORCE (don't start if the plugin
                      fails to load).
  --innodb-ft-inserted[=name]
                      Enable or disable INNODB_FT_INSERTED plugin. Possible
                      values are ON, OFF, FORCE (don't start if the plugin
                      fails to load).
  ...
  --innodb-old-blocks-time=#
                      ...
                      timeout is disabled if 0 (the default).
  --innodb-open-files=#
  ...
  --innodb-use-atomic-writes
                      Prevent partial page writes, via atomic writes.The option
                      is used to prevent partial writes in case of a
                      crash/poweroff, as faster alternative to doublewrite
                      buffer.Currently this option works only on Linux only
                      with FusionIO device, and directFS filesystem.
  --innodb-use-fallocate
                      Preallocate files fast, using operating system
                      functionality. On POSIX systems, posix_fallocate system
                      call is used.
  ...
  --sort-buffer-size=#
                      ...
                      this size
  --sphinx[=name]     Enable or disable SPHINX plugin. Possible values are ON,
                      OFF, FORCE (don't start if the plugin fails to load).
...
aria-sort-buffer-size                                      134217728
...
blackhole                                                  ON
bulk-insert-buffer-size                                    16777216
...
innodb-analyze-is-persistent                               FALSE
...
myisam-sort-buffer-size                                    536870912
